...
class PCF:
    ...
    def filtration(self, rule, nd = None, rips_max = None):
        ...
        if rule == 'alpha':
            self.f = Filtration()
            if self.nd == 2:
                fill_alpha2D_complex(self.pts.tolist(), self.f)
            elif self.nd == 3:
                fill_alpha3D_complex(self.pts.tolist(), self.f)
            self.f.sort(dim_data_cmp)
            self.rule = 'alpha'
        elif rule == 'rips_euc':
            self.dmat = PairwiseDistances(self.pts.tolist())
            rips = Rips(self.dmat)
            self.f = Filtration()
            rips.generate(self.phnd, rips_max, self.f.append)
            for s in self.f: s.data = [rips.eval(s)]
            self.f.sort(dim_data_cmp)
            self.rule = 'rips_euc'
        elif rule == 'rips_dmat':
            tmp_dmat = self.dmat
            self.dmat = DefinedDistance(tmp_dmat)
            rips = Rips(self.dmat)
            self.f = Filtration()
            rips.generate(self.phnd, rips_max, self.f.append)
            for s in self.f: s.data = [rips.eval(s)]
        ...
        for s in self.f:
            if self.rule == 'alpha':
                self.f_values.append(sqrt(s.data[0]))
            elif self.rule == 'rips_euc' or self.rule == 'rips_dmat':
                self.f_values.append(s.data[0])
            tmp_nodes = str(s)[1:-1].split(',')
            nodes = []
    ...
    def pcoh(self, prime=547):
        ...
        """
        self.prime = prime
        self.p = StaticCohomologyPersistence(self.f, prime = prime)
        self.p.pair_simplices()
        if self.rule == 'alpha':
            self.dgms = init_diagrams(self.p, self.f, \
                        lambda s: sqrt(s.data[0]), lambda n: n.cocycle)
        else:
            self.dgms = init_diagrams(self.p, self.f, \
                        lambda s: s.data[0], lambda n: n.cocycle)
...
def main():
    ...
    a.construct_smoothed_cocycles(1, 0.005, "combinatorial_full", nsp=10)
